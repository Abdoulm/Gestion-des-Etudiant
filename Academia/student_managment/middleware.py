    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if user.is_authenticated:
            if user.user_type == '1': # Is it the HOD/Admin
                if modulename == 'student_managment.student_views':
                    return redirect(reverse('admin_home'))
            elif user.user_type == '2': #  Staff :-/ ?
                if modulename == 'student_managment.student_views' or modulename == 'student_managment.hod_views':
                    return redirect(reverse('staff_home'))
            elif user.user_type == '3': # ... or Student ?
                if modulename == 'student_managment.hod_views' or modulename == 'student_managment.staff_views':
